--======================================================================================
-- DOMINATE HUB | PRO EDITION (STABLE V17.0 - WIDER ARENA RANGE & INSTANT SNAPPING)
--======================================================================================
        ...
        if hrp and Running then
            ...
            if act then
                ...
                if (hrp.Position - act).Magnitude > 0.3 then
                    hrp.Anchored = false
                    -- Boost trial snap speed for instant target acquisition
...
-- TIME-BASED SCHEDULED TRIAL AUTOMATION (:29 and :59 past every hour) - WIDER ARENA RANGE & FAST SNAP
        ...
        if Running and (Env.AutoEasyTrial or Env.AutoMediumTrial or Env.AutoHardTrial) and not ritualIsActive then
            ...
            if (min == 29 or min == 59) and min ~= lastTrialTriggeredMinute then
                ...
                if targetPad then
                    ...
                    -- 3. Turn on mob attack / start clearing arena mobs with expanded range (250 studs) to catch every single mob
                    ...
                    while clearingActive and Running do
                        ...
                        if closestMobPart then
                            ...
                            if mobsFoundCount == 0 then
                                ...
                                if noMobCounter >= 15 then -- ~3 seconds of zero mobs means trial cleared
...
print("[Dominate Hub] V17.0 Wider Arena Range & Fast Snap Loaded Successfully!")pcall(function()
    local blur = Instance.new("BlurEffect")
    blur.Name = "DominateHubBlur"
    blur.Size = 6
    blur.Parent = Lighting
                ...
                if targetPad then
                    ...
                    while clearingActive and Running do
                        ...
                        if trialsFolder and hrp then
                            for _, roomObj in ipairs(trialsFolder:GetChildren()) do
                                for _, mobObj in ipairs(roomObj:GetDescendants()) do
                                    -- Ensure we only target valid enemy mob names and ignore scenery/bushes
                                    if mobObj:IsA("Model") and validMobLookup[mobObj.Name] and not isMobRespawning(mobObj) then
                                        local part = mobObj.PrimaryPart or mobObj:FindFirstChildWhichIsA("BasePart")
                                        if part then
                                            mobsFoundCount = mobsFoundCount + 1
...
print("[Dominate Hub] V16.9 Syntax Verified Loaded Successfully!")
