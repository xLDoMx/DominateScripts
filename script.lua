--======================================================================================
-- DOMINATE HUB | PRO EDITION (SAFE BREAK MEAT DEPOSIT, DUNES RUNE & STABLE BUILD)
--======================================================================================
local Env = getgenv()
...
-- SAFE ZONE COMBAT BREAK ROUTINE (2S TOTAL PAUSE, SAFE SPOT GLIDE & MEAT DEPOSIT)
task.spawn(function()
    while Running do
        task.wait(40.0)
        if Running and Env.AutoCombatBreak then
            ...
            if anyActive then
                ...
                -- Trigger DepositMeat during combat safe break if enabled
                if NetRemote and (Env.AutoDepositMeat or Env.AutoMeatMoreMeat) then
...
-- 1-MINUTE MEAT TO BONES CONVERSION LOOP
task.spawn(function() 
    while Running do 
        task.wait(60.0) 
        if NetRemote and Running then 
            if Env.AutoDepositMeat or Env.AutoMeatMoreMeat then 
                pcall(function() NetRemote:FireServer("DepositMeat") end) 
            end 
        end 
    end 
...
print("[Dominate Hub] V11.67 1-Minute Meat Conversion & Safe Spot Break Loaded Successfully!")        corner.Parent = toast
...
print("[Dominate Hub] V11.65 Merchant Noob, Extended Bones Upgrades & Master Toggles Loaded Successfully!")
