--======================================================================================
-- DOMINATE HUB | PRO EDITION (STABLE V16.9.103 - FULLY CLEANED & FIXED)
--======================================================================================
...
-- ROBUST ORE RESPAWNING / COMPLETION CHECK
    ...
    while Running do
        ...
        if Running and not trialIsRunning and not trialExiting and not ritualIsActive then
            ...
            if hasAnyOreEnabled then
                ...
                -- Keep lock on current ore UNLESS it is fully respawning/depleted OR stuck past 30 seconds
                if currentTargetPanel and currentTargetPanel.Parent then
                    local oreModel = currentTargetPanel.Parent
                    if oreModel:IsA("Model") and not isOreRespawning(oreModel) and (now - oreLockStartTime) <= 30.0 then
                        currentValid = true
                        ...
                        for i = #OrePriorityList, 1, -1 do
                            ...
                            if enabledOreNames[targetOreName] then
                                ...
                                if #tierOres > 0 then
                                    local hrp = GetWorldRoot()
                                    if hrp then
                                        table.sort(tierOres, function(a, b)
                                            return (a.Position - hrp.Position).Magnitude < (b.Position - hrp.Position).Magnitude
                                        end)
                                    end
                                    foundOrePart = tierOres[1]
                                    break
                                end
                            end
                        end
                ...
                if currentTargetPanel ~= oldTarget then
                    oreLockStartTime = tick() -- Reset lock timer when switching to a new ore
                end
                    ...
                    for i = 1, #vis do
                        ...
                        if bM and bM:IsA("Model") and not locked[bM.Name] then
                            ...
                            if tb and tb:IsA("BasePart") and Running then
                                att = true 
                                MasterTargetVector = tb.Position + Vector3.new(0, 1, 0)
                                task.wait(0.4)
...
```[cite: 1]    AncientCap = Vector3.new(714.6417236328125, 4.870510101318359, 7814.7265625),
    Dunes = Vector3.new(981.1582, 4.5862, 7767.3315),
    SandPit = Vector3.new(552.6134, 3.9798, 7827.5971),
    Sunfire = Vector3.new(692.3831176757812, 4.754001617431641, 7735.392578125),
    EasyTrial = Vector3.new(850.21044921875, 11.162318229675293, 13444.673828125),     
