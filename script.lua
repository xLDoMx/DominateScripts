--======================================================================================
-- DOMINATE HUB | PRO EDITION (STABLE V16.9.23 - PER-WAVE ANTI-STUCK TIMER & SAFE EXIT)
--======================================================================================
...
-- SCHEDULED TRIAL AUTOMATION (WITH PER-WAVE 60S ANTI-STUCK TIMER & SAFE SURFACE TELEPORT)
            ...
            if (min == 29 or min == 59) and min ~= lastTrialTriggeredMinute then
                ...
                if targetPad then
                    ...
                    while Running and trialIsRunning do
                        ...
                        -- Per-wave anti-stuck timer check (resets automatically every wave)
                        pcall(function()
                            for _, lbl in ipairs(player.PlayerGui:GetDescendants()) do
                                if lbl:IsA("TextLabel") and lbl.Text and lbl.Text:lower():find("wave") then
                                    if lbl.Text ~= currentWaveText then
                                        ...
                                        waveTimer = tick() -- Reset timer on new wave advance!
                                    end
                                    break
                                end
                            end
                        ...
                        pcall(function()
                            ...
                            local hrp = GetWorldRoot()
                            if hrp and hrp.Position.Z > 13000 then
                                -- Active inside trial arena
                            else
                    ...
                    -- Safe surface relocation: Teleport back to Castle Entrance to prevent getting stuck in lobby
...
-- ZERO-DELAY UNIFIED MOB & CORPSE-SKIPPING FARMING ENGINE
task.spawn(function()
    while Running do
        task.wait(0.01) -- 10ms super-tight poll rate for instantaneous reaction on mob death
        if Running and not ritualIsActive and not ritualSuppressMobs and RitualTargetVector == nil and SandTargetVector == nil then
            ...
            if hasAnyMobEnabled then
                ...
                elseif currentTargetMob.Parent and isMobRespawning(currentTargetMob.Parent) then 
                    ...
                    local model = currentTargetMob:FindFirstAncestorWhichIsA("Model")
                    local hum = model and model:FindFirstChildOfClass("Humanoid")
                    if hum and hum.Health <= 0 then
                        if model then deadMobs[model] = true end
                        needsNewMobTarget = true
                    end
                end
...
print("[Dominate Hub] V16.9.23 Stable Loaded Successfully!")end)
...
-- SCHEDULED TRIAL AUTOMATION (LOCKS TOGGLES & CLEARS ALL WAVES UNTIL TRIAL ENDS)
local lastTrialTriggeredMinute = -1
task.spawn(function()
    while Running do
        task.wait(1.0)
...
print("[Dominate Hub] V16.9.21 Stable Loaded Successfully!")pcall(function()
    local blur = Instance.new("BlurEffect")
    blur.Name = "DominateHubBlur"
    blur.Size = 6
    blur.Parent = Lighting
