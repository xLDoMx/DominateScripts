--======================================================================================
-- DOMINATE HUB | PRO EDITION (STABLE V16.9.10 - VERIFIED TRIAL BUSH FIX & 250 LAYERS)
--======================================================================================
...
-- TIME-BASED SCHEDULED TRIAL AUTOMATION (:29 and :59) - USES BILLBOARD + HUMANOID FILTER
...
print("[Dominate Hub] V16.9.10 Stable Loaded Successfully!")pcall(function()
    local blur = Instance.new("BlurEffect")
    blur.Name = "DominateHubBlur"
    blur.Size = 6
    blur.Parent = Lighting
...
-- TIME-BASED SCHEDULED TRIAL AUTOMATION (:29 and :59 past every hour) - 200 STUDS + VALID MOB LOOKUP (NO BUSH STICKING)
local lastTrialTriggeredMinute = -1
task.spawn(function()
    while Running do
        task.wait(1.0)
        if Running and (Env.AutoEasyTrial or Env.AutoMediumTrial or Env.AutoHardTrial) and not ritualIsActive then
            ...
            if (min == 29 or min == 59) and min ~= lastTrialTriggeredMinute then
                ...
                if targetPad then
                    ...
                    -- 3. Turn on mob attack with 200-stud range and valid mob name filter (ignores bushes entirely)
                    showToast("Trials: 60s elapsed! Turning on mob attack and clearing arena...")
                    local clearingActive = true
                    local noMobCounter = 0
...
print("[Dominate Hub] V16.9.8 Stable Loaded Successfully!")
